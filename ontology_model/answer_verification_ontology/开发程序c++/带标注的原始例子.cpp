...
void print_graph()
{
    ...
    if (SC_TRUE == sc_iterator5_next(it)) {
        ...
        while (SC_TRUE == sc_iterator3_next(arcs_it)) {
            ...
            printEl(context, v2);
            cout << endl;
            //如果弧两端的节点不在则把他们加入变量print_vertex
    ...
    if (SC_TRUE == sc_iterator5_next(it)) {
        ...
        while (SC_TRUE == sc_iterator3_next(nodes_it)) {
            ...
            //在图中找到的定点在集合print_vertex中吗,如果不在则打印该定点
...
void run_test(char number_test, sc_char *beg_vertex, sc_char *end_vertex)
{
    ...
    //设置长度为三的字符串，并且字符串第二个位置字符可以按照G0，G1等等更改，其中字符串gr代表图的系统标识符，将系统标识符的地址赋值给graph.
    //*graph=G0,G1,G2,...
    char gr[3] = "Gx";
    gr[1] = number_test;
